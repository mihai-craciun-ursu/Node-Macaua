...
var cards = (function() {
	...
	function init(options) {
		...
		if (opt.blackJoker) {
			all.push(new Card('j', 0, opt.table));
		}
		if (opt.redJoker) {
			all.push(new Card('l', 0, opt.table));
		}
	...
	Card.prototype = {
		...
		showCard : function() {
			var offsets = { "c": 0, "d": 1, "h": 2, "s": 3, "j":2, "l":3 };
			var xpos, ypos;
			var rank = this.rank;
			if (rank == 14) {
	...
	Container.prototype.extend({
		...
		findCard: function(suit, rank){
			for(var i=0; i<this.length; i++)
				if(this[i].suit == suit && this[i].rank == rank)
					return this[i];
		},
